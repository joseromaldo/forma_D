...
if (isset($_GET['asi_id'])) {
    $_GET['asi_id'] = filter_var(base64_decode($_GET['asi_id']), FILTER_SANITIZE_NUMBER_INT);
} else {
    $resultado = [
        'mensaje' => 'ID NO VÁLIDO',
        'detalle' => 'El ID de la asignación no está presente o es inválido.',
        'codigo' => 0
    ];
}

try {
    if (isset($_GET['asi_id']) && $_GET['asi_id']) {
        $asignacion = new Asignar(['asi_id' => $_GET['asi_id']]);
        ...
        throw new Exception('ID de asignación no válido o no proporcionado');
    }
} catch (PDOException $e) {
 
    if ($e->getCode() == '23000') {
        $resultado = [
            'mensaje' => 'No se puede eliminar la asignación porque está siendo referenciada.',
            'detalle' => '',
            'codigo' => 0
        ];
    } else {
